		...
		if constexpr (NW_TRUTH) {
			...
			if constexpr (NW_TRUTH)
			{
				...
				if constexpr (NW_FALSE) {
					...
					auto& ibuf = cmp_sys::get().new_ref<gfx_buf_idx>(t_mem_layt<v1u>("index"), 6u).get_val<gfx_buf>();
					if constexpr (NW_TRUTH) {
						if constexpr (NW_TRUTH) {
							for (v1u ivert = 0u; ivert < vbuf.get_count(); ivert++) {
			...
			case NW_EVTYPE_KEYBOD_RAISE:
				...
				case NW_KEYCODE_M: {
					...
					if (!app_engine::get().is_cursor_enabled()) {
						app_engine::get().set_cursor_enabled(NW_TRUTH);
...
namespace NW
{
	...
	{
		auto context = gfx_engine::get().get_context();
		NW_CHECK(
			gui_init(
				context.get_window(), context.get_device(), context.get_handle()
			), "failed init", return NW_FALSE
		);

		gui_ref ref;
		
		ref.make_ref<gui_of_app_engine>();
		ref->set_enabled(NW_TRUTH);
