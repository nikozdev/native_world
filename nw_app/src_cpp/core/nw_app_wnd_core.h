...
namespace NW
{
	...
	public:
		...
		using raw_buf_t = darray_t<sbyte_t>;
	public:
		app_wnd_core();
		...
		inline v1bit remake(cstr_t title, cv1u dims, cv1u size_y) { set_title(title); set_size_x(dims); set_size_y(size_y); return remake(); }
