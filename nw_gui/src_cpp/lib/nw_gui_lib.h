...
#define IMGUI_CHECKVERSION()        NW_GUI::DebugCheckVersionAndDataLayout(IMGUI_VERSION, sizeof(ImGuiIO), sizeof(ImGuiStyle), sizeof(ImVec2), sizeof(ImVec4), sizeof(ImDrawVert), sizeof(ImDrawIdx))
#define IMGUI_HAS_TABLE
#define IMGUI_HAS_VIEWPORT          // Viewport WIP branch
#define IMGUI_HAS_DOCK              // Docking WIP branch

// Define attributes of all API symbols declarations (e.g. for DLL under Windows)
// IMGUI_API is used for core imgui functions, IMGUI_IMPL_API is used for the default backends files (imimpl_xxx.h)
...
struct ImGuiContext;                // Dear ImGui context (opaque structure, unless including iminternal.h)
...
typedef int (*ImGuiInputTextCallback)(ImGuiInputTextCallbackData* data);    // Callback function for NW_GUI::InputText()
typedef void (*ImGuiSizeCallback)(ImGuiSizeCallbackData* data);             // Callback function for NW_GUI::SetNextWindowSizeConstraints()
typedef void* (*ImGuiMemAllocFunc)(size_t sz, void* user_data);             // Function signature for NW_GUI::SetAllocatorFunctions()
typedef void (*ImGuiMemFreeFunc)(void* ptr, void* user_data);               // Function signature for NW_GUI::SetAllocatorFunctions()
...
//-----------------------------------------------------------------------------
// [SECTION] Dear ImGui end-user API functions
// (Note that NW_GUI:: being a namespace, you can add extra NW_GUI:: functions in your own separate file. Please don't modify imgui source files!)
//-----------------------------------------------------------------------------
...
namespace NW_GUI
{
    ...
    // - If you want to use InputText() with std::string or any custom dynamic string type, see misc/cpp/imstdlib.h and comments in imdemo.cpp.
    ...
    IMGUI_API void          SetTooltip(const char* fmt, ...) IM_FMTARGS(1);                     // set a text-only tooltip, typically use with NW_GUI::IsItemHovered(). override any previous call to SetTooltip().
    ...
    // - See top of imtables.cpp for general commentary.
    ...
    // Clipping
    // - Mouse hovering is affected by NW_GUI::PushClipRect() calls, unlike direct calls to ImDrawList::PushClipRect() which are render only.
    ...
    IMGUI_API int           GetKeyIndex(ImGuiKey imkey);                                    // map ImGuiKey_* values into user's key index. == io.KeyMap[key]
    ...
    IMGUI_API ImVec2        GetMousePos();                                                      // shortcut to NW_GUI::GetIO().MousePos provided by user, to be consistent with other calls
    ...
    IMGUI_API ImGuiMouseCursor GetMouseCursor();                                                // get desired cursor type, reset in NW_GUI::NewFrame(), this is updated during the frame. valid before Render(). If you use software rendering by setting io.MouseDrawCursor ImGui will render those for you
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_HorizontalScrollbar    = 1 << 11,  // Allow horizontal scrollbar to appear (off by default). You may use SetNextWindowContentSize(ImVec2(width,0.0f)); prior to calling Begin() to specify width. Read code in imdemo in the "Horizontal Scrolling" section.
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_CallbackResize      = 1 << 18,  // Callback on buffer capacity changes request (beyond 'buf_size' parameter value), allowing the string to grow. Notify when the string wants to be resized (for string types which hold a cache of their Size). You will be provided a new BufSize in the callback and NEED to honor it. (see misc/cpp/imstdlib.h for an example of using this)
...
// Flags for NW_GUI::TreeNodeEx(), NW_GUI::CollapsingHeader*()
...
// - Read on documentation at the top of imtables.cpp for details.
...
// Flags for NW_GUI::TableSetupColumn()
...
// Enum for NW_GUI::TableSetBgColor()
...
// Flags for NW_GUI::IsWindowFocused()
...
// Flags for NW_GUI::IsItemHovered(), NW_GUI::IsWindowHovered()
...
// Flags for NW_GUI::DockSpace(), shared/inherited by child nodes.
...
// Flags for NW_GUI::BeginDragDropSource(), NW_GUI::AcceptDragDropPayload()
...
// Backend capabilities flags stored in io.BackendFlags. Set by imimpl_xxx or custom backend.
...
// Flags for InvisibleButton() [extended in iminternal.h]
...
// Enumeration for NW_GUI::SetWindow***(), SetNextWindow***(), SetNextItem***() functions
...
#define IM_ALLOC(_SIZE)                     NW_GUI::MemAlloc(_SIZE)
#define IM_FREE(_PTR)                       NW_GUI::MemFree(_PTR)
#define IM_PLACEMENT_NEW(_PTR)              new(ImNewWrapper(), _PTR)
#define IM_NEW(_TYPE)                       new(ImNewWrapper(), NW_GUI::MemAlloc(sizeof(_TYPE))) _TYPE
template<typename T> void IM_DELETE(T* p)   { if (p) { p->~T(); NW_GUI::MemFree(p); } }
...
// [SECTION] ImGuiStyle
//-----------------------------------------------------------------------------
// You may modify the NW_GUI::GetStyle() main instance during initialization and before NewFrame().
// During the frame, use NW_GUI::PushStyleVar(ImGuiStyleVar_XXXX)/PopStyleVar() to alter the main style values,
// and NW_GUI::PushStyleColor(ImGuiCol_XXX)/PopStyleColor() for colors.
//-----------------------------------------------------------------------------
...
// Access via NW_GUI::GetIO(). Read 'Programmer guide' section in .cpp file for general usage.
//-----------------------------------------------------------------------------
...
struct ImGuiIO
{
    ...
    ImGuiBackendFlags  BackendFlags;            // = 0              // See ImGuiBackendFlags_ enum. Set by backend (imimpl_xxx files or custom backend) to communicate features supported by the backend.
    ...
    const char* LogFilename;                    // = "imlog.txt"// Path to .log file (default parameter to NW_GUI::LogToFile when no file is specified).
    ...
    //------------------------------------------------------------------
    // Platform Functions
    // (the imimpl_xxxx backend files are setting those up for you)
    //------------------------------------------------------------------
...
// Usage: static ImGuiOnceUponAFrame oaf; if (oaf) NW_GUI::Text("This will be called only once per frame");
struct ImGuiOnceUponAFrame
{
    ImGuiOnceUponAFrame() { RefFrame = -1; }
    mutable int RefFrame;
    operator bool() const { int current_frame = NW_GUI::GetFrameCount(); if (RefFrame == current_frame) return false; RefFrame = current_frame; return true; }
};
...
struct ImGuiStorage
{
    ...
    //      float* pvar = NW_GUI::GetFloatRef(key); NW_GUI::SliderFloat("var", pvar, 0, 100.0f); some_var += *pvar;
...
struct ImColor
{
    ...
    inline operator ImU32() const                                   { return NW_GUI::ColorConvertFloat4ToU32(Value); }
    inline operator ImVec4() const                                  { return Value; }

    // FIXME-OBSOLETE: May need to obsolete/cleanup those helpers.
    inline void    SetHSV(float h, float s, float v, float a = 1.0f){ NW_GUI::ColorConvertHSVtoRGB(h, s, v, Value.x, Value.y, Value.z); Value.w = a; }
    static ImColor HSV(float h, float s, float v, float a = 1.0f)   { float r, g, b; NW_GUI::ColorConvertHSVtoRGB(h, s, v, r, g, b); return ImColor(r, g, b, a); }
};
...
// Flags for ImDrawList instance. Those are set automatically by NW_GUI:: functions from ImGuiIO settings, and generally not manipulated directly.
...
// Draw command list
// This is the low-level list of polygons that NW_GUI:: functions are filling. At the end of the frame,
// all command lists are passed to your ImGuiIO::RenderDrawListFn function for rendering.
// Each dear imgui window contains its own ImDrawList. You can use NW_GUI::GetWindowDrawList() to
// access the current window draw list and draw custom primitives.
// You can interleave normal NW_GUI:: calls and adding primitives to the current draw list.
// In single viewport mode, top-left is == GetMainViewport()->Pos (generally 0,0), bottom-right is == GetMainViewport()->Pos+Size (generally io.DisplaySize).
// You are totally free to apply whatever transformation matrix to want to the data (depending on the use of the transformation you may want to apply it to ClipRect as well!)
// Important: Primitives are always added to the list and not culled (culling is done at higher-level by NW_GUI:: functions), if you use this API a lot consider coarse culling your drawn objects.
struct ImDrawList
{
    ...
    const ImDrawListSharedData* _Data;          // Pointer to shared draw data (you can use NW_GUI::GetDrawListSharedData() to get the one from current ImGui context)
    ...
    // If you want to create ImDrawList instances, pass them NW_GUI::GetDrawListSharedData() or create and use your own ImDrawListSharedData (so you can use ImDrawList without ImGui)
    ImDrawList(const ImDrawListSharedData* shared_data) { memset(this, 0, sizeof(*this)); _Data = shared_data; }

    ~ImDrawList() { _ClearFreeMemory(); }
    IMGUI_API void  PushClipRect(ImVec2 clip_rect_min, ImVec2 clip_rect_max, bool intersect_with_current_clip_rect = false);  // Render-level scissoring. This is passed down to your render function but not used for CPU-side coarse clipping. Prefer using higher-level NW_GUI::PushClipRect() to affect logic (hit-testing and widget culling)
...
//  - Upload the pixels data into a texture within your graphics system (see imimpl_xxxx.cpp examples)
...
struct ImFontAtlas
{
    ...
    bool                        Locked;             // Marked as Locked by NW_GUI::NewFrame() so attempt to modify the atlas will assert.
    ...
    ImVector<ImFont*>           Fonts;              // Hold all the fonts returned by AddFont*. Fonts[0] is the default font upon calling NW_GUI::NewFrame(), use NW_GUI::PushFont()/PopFont() to change the current font.
...
// - So e.g. NW_GUI::SetNextWindowPos(ImVec2(0,0)) will position a window relative to your primary monitor!
// - If you want to position windows relative to your main application viewport, use NW_GUI::GetMainViewport()->Pos as a base position.
//
// Steps to use multi-viewports in your application, when using a default backend from the examples/ folder:
// - Application:  Enable feature with 'io.ConfigFlags |= ImGuiConfigFlags_ViewportsEnable'.
// - Backend:      The backend initialization will setup all necessary ImGuiPlatformIO's functions and update monitors info every frame.
// - Application:  In your main loop, call NW_GUI::UpdatePlatformWindows(), NW_GUI::RenderPlatformWindowsDefault() after EndFrame() or Render().
// - Application:  Fix absolute coordinates used in NW_GUI::SetWindowPos() or NW_GUI::SetNextWindowPos() calls.
//
...
// - Application:  Fix absolute coordinates used in NW_GUI::SetWindowPos() or NW_GUI::SetNextWindowPos() calls.
//
// About NW_GUI::RenderPlatformWindowsDefault():
...
// (Optional) Access via NW_GUI::GetPlatformIO()
struct ImGuiPlatformIO
{
    ...
    //   N = NW_GUI::NewFrame()                        ~ beginning of the dear imgui frame: read info from platform/OS windows (latest size/position)
    //   F = NW_GUI::Begin(), NW_GUI::EndFrame()        ~ during the dear imgui frame
    //   U = NW_GUI::UpdatePlatformWindows()           ~ after the dear imgui frame: create and update all platform/OS windows
    //   R = NW_GUI::RenderPlatformWindowsDefault()    ~ render
    //   D = NW_GUI::DestroyPlatformWindows()          ~ shutdown
    // The general idea is that NewFrame() we will read the current Platform/OS state, and UpdatePlatformWindows() will write to it.
    //
    // The functions are designed so we can mix and match 2 imimpl_xxxx files, one for the Platform (~window/input handling), one for Renderer.
    ...
    // Viewports list (the list is updated by calling NW_GUI::EndFrame or NW_GUI::Render)
...
// Include imuser.h at the end of imgui.h (convenient for user to only explicitly include vanilla imgui.h)
