#ifndef NW_GUI_INTERNAL_H
#define NW_GUI_INTERNAL_H
...
// [SECTION] Test Engine specific hooks (imtest_engine)
...
#error Must include imgui.h before iminternal.h
#endif
...
// Helpers: Maths
// - Wrapper for standard libs functions. (Note that imdemo.cpp does _not_ use them to keep the code easy to copy)
...
// For a given item ID, access with NW_GUI::GetInputTextState()
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImDrawList*             DrawList;                           // == &DrawListInst (for backward compatibility reason with code using iminternal.h we keep this a pointer)
...
namespace NW_GUI
{
    ...
    // - NW_GUI::NewFrame() has never been called, which is illegal.
    // - You are calling ImGui functions after NW_GUI::EndFrame()/NW_GUI::Render() and before the next NW_GUI::NewFrame(), which is also illegal.
    ...
    // Basic Accessors
    inline ImGuiID          GetItemID()     { ImGuiContext& g = *GImGui; return g.CurrentWindow->DC.LastItemId; }   // Get ID of last item (~~ often same NW_GUI::GetID(label) beforehand)
    ...
    // Template functions are instantiated in imwidgets.cpp for a finite number of types.
