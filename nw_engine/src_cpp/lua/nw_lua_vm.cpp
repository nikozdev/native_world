...
namespace NW
{
	...
	template <> Int32 LuaVM::GetLuaValueLoc<Int32>(const char* strName) {
		Int32 nRes = 0;
	...
	template <> Int32 LuaVM::GetLuaValue<Int32>(const char* strLoc, const char* strName) {
		Int32 nRes = 0;
	...
	template <> bool LuaVM::SetLuaValueLoc<Int32>(const Int32& tValue, const char* strName) {
	...
	template <> bool LuaVM::SetLuaValue<Int32>(const Int32& tValue, const char* strLoc, const char* strName) {
	...
	LuaState* LuaVM::CreateLuaState()
	{
		...
		//LuaState* pLState = lua_newstate(LuaVM::LuaAllocate, &m_MemAllocator);
		LuaState* pLState = luaL_newstate();
		//lua_setallocf(pLState, LuaVM::LuaAllocate, &m_MemAllocator);
	...
	void LuaVM::DestroyLuaState(LuaState* pLState)
	{
		lua_close(pLState);
		m_MemAllocator.Reset(0);
	}

	bool LuaVM::LoadScript(const String& strScript)
	{
		LuaReaderState lRState;
		lRState.strChunk = &strScript;
		Int32 nResult = 0;//lua_load(m_LState, LuaVM::LuaRead, &lRState, "lua_chunk", "r");
		return nResult == LUA_OK;
	}

	bool LuaVM::RunScriptStr(const char* strScript)
	{
		bool bResult = true;
	...
	bool LuaVM::RunScriptF(const char* strFPath)
	{
		bool bResult = true;
	...
	bool LuaVM::CallFunctionGlb(const char* strName)
	{
		bool bResult = true;
	...
	bool LuaVM::CallFunctionLoc(const char* strName)
	{
		bool bResult = true;
	...
	bool LuaVM::CallFunction(const char* strLoc, const char* strName)
	{
		bool bResult = true;
	...
	bool LuaVM::LuaReg(LuaRegFunc& rRegFunc)
	{
	...
	bool LuaVM::LuaReg(LuaRegTable& rRegTab)
	...
	inline bool LuaVM::LuaMoveLoc(const char* strName)
	{
	...
	inline bool LuaVM::LuaMoveTGlb(const char* strName, LuaTypes nLuaType) {
		if (LuaMoveGlb(strName)) { return m_Info.GetLastType() == nLuaType; }
		else { return false; }
	}
	inline bool LuaVM::LuaMoveTLoc(const char* strName, LuaTypes nLuaType) {
		if (LuaMoveLoc(strName)) { return m_Info.GetLastType() == nLuaType; }
		else { return false; }
	}
	inline bool LuaVM::LuaMoveT(const char* strLoc, const char* strName, LuaTypes nLuaType) {
		if (LuaMove(strLoc, strName)) { return m_Info.GetLastType() == nLuaType; }
		else { return false; }
	}

	inline void LuaVM::LuaClear() {
		UInt8 unLevel = lua_gettop(m_LState);
		LuaClear(unLevel);
	}
	inline void LuaVM::LuaClear(UInt8 unCount)
	{
	...
	inline bool LuaVM::LuaPush(LuaRegFunc& rRegFunc)
	{
	...
	inline bool LuaVM::LuaPush(LuaRegTable& rRegTab)
	{
	...
	inline void LuaVM::WriteErr(const char* strFormat, ...)
	{
		va_list valArgs;
	...
	const char* LuaVM::LuaRead(LuaState* pState, void* pData, Size* szData)
	{
	...
	void* LuaVM::LuaAllocate(void* pAllocator, void* pBlock, Size szOld, Size szNew)
	{
...
//inline const LuaRegFunc* LuaVM::GetRegFunction(const char* strName) {
//	RegFunctions::iterator itRegFunc = m_RegFunctions.find(strName);
//	return itRegFunc == m_RegFunctions.end() ? nullptr : &itRegFunc->second;
//}
//inline const LuaRegTable* LuaVM::GetRegTable(const char* strName) {
...
//}
//void LuaVM::RemoveRegFunction(const char* strName) {
//	RegFunctions::iterator itRegFunc = m_RegFunctions.find(strName);
//	if (itRegFunc == m_RegFunctions.end()) { return; }
//	m_RegFunctions.erase(itRegFunc);
//}
//void LuaVM::RemoveRegTable(const char* strName) {
//	RegTables::iterator itRegTab = m_RegTables.find(strName);
//	if (itRegTab == m_RegTables.end()) { return; }
//	m_RegTables.erase(itRegTab);
//}
// -- Predicates
//bool LuaVM::HasTable(const char* strName) {
//	UInt16 unLevel = LuaMove(strName);
//	bool bHasTab = lua_type(m_LState, -1) == LUA_TTABLE;
//	LuaClear(unLevel);
//	return bHasTab;
//}
//bool LuaVM::HasRegTable(const char* strName) { return m_RegTables.find(strName) != m_RegTables.end(); }
//bool LuaVM::HasFunction(const char* strName) {
//	UInt16 unLevel = LuaMove(strName);
//	bool bHasFunc = lua_type(m_LState, -1) == LUA_TFUNCTION;
//	LuaClear(unLevel);
//	return bHasFunc;
//}
//bool LuaVM::HasRegFunction(const char* strName) { return m_RegFunctions.find(strName) != m_RegFunctions.end(); }
...
//void LuaVM::AddRegTable(LuaRegTable& rRegTab) {
//	String strAbsName = "";
//	UInt16 unLevel = 0;
