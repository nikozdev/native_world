...
namespace NW
{
	...
	// --getters
	template <> v1si lua_engine::get_lua_value_glb<v1si>(cstr name) {
		v1si result = 0;
		if (lua_move_tglb(name, LT_NUM)) {
			result = static_cast<v1si>(lua_tonumberx(m_lstate, -1, 0));
		}
		else { result = 0; }
		lua_clear(1);
		return result;
	}
	template <> v1si lua_engine::get_lua_value_loc<v1si>(cstr name) {
		v1si result = 0;
	...
	template <> v1si lua_engine::get_lua_value<v1si>(cstr strLoc, cstr name) {
		v1si result = 0;
	...
	// --setters
	template <> bool lua_engine::set_lua_value_glb<v1si>(const v1si& tValue, cstr name) {
	...
	template <> bool lua_engine::set_lua_value_loc<v1si>(const v1si& tValue, cstr name) {
	...
	template <> bool lua_engine::set_lua_value<v1si>(const v1si& tValue, cstr strLoc, cstr name) {
	...
	inline void lua_engine::lua_clear() {
		v1ui lvl_num = lua_gettop(m_lstate);
		lua_clear(lvl_num);
	}
	inline void lua_engine::lua_clear(v1si count)
	{
	...
	inline bool lua_engine::lua_push(lua_func_info& reg_info)
	{
		...
		for (v1ui uvi = 0; uvi < up_vals_cpy.size(); uvi++) { // throw all the upvalues on the top of the stack
	...
	{
		v1si lvl_num = lua_gettop(m_lstate);
		m_info.rmv_type(lvl_num);
		for (v1si ti = 0; ti < lvl_num; ti++) { m_info.add_type(lua_type(m_lstate, -ti)); }
...
//		v1si ltype = lua_type(m_lstate, -1);
//		if (ltype == LUA_TTABLE) {
