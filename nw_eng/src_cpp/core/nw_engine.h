...
namespace NW
{
	...
	public:
		...
		using window = mem_ref<wnd_core>;
		using graphics = wnd_core::graphics;
		using keyboard = wnd_core::keyboard;
		using mouse = wnd_core::mouse;
		using timer = time_state;
		using kbd_code = keyboard_codes;
		using msb_code = mouse_codes;
	public:
		core_engine(cstr name = "nw_engine");
		...
		inline state& get_state(v1ui idx)   { return m_states[idx % m_states.size()]; }
		...
		// --setters
		void set_cursor_enabled(bit enable);
		template<class stype, typename ... args>
		state& new_state(args ... arguments);
		void del_state(v1ui idx);
		void stop_running();
		// --predicates
		inline bit is_running() const                    { return m_is_running; }
		inline bit is_cursor_enabled() const             { return m_wnd->is_cursor_enabled(); }
		inline bit is_mouse_released(msb_code code)const { return m_wnd->get_mouse()->is_released(code); }
		inline bit is_mouse_pressed(msb_code code) const { return m_wnd->get_mouse()->is_pressed(code); }
		inline bit is_mouse_free(msb_code code) const    { return m_wnd->get_mouse()->is_free(code); }
		inline bit is_mouse_held(msb_code code) const    { return m_wnd->get_mouse()->is_held(code); }
		inline bit is_key_released(kbd_code code) const  { return m_wnd->get_keyboard()->is_released(code); }
		inline bit is_key_pressed(kbd_code code) const   { return m_wnd->get_keyboard()->is_pressed(code); }
		inline bit is_key_free(kbd_code code) const      { return m_wnd->get_keyboard()->is_free(code); }
		inline bit is_key_held(kbd_code code) const      { return m_wnd->get_keyboard()->is_held(code); }
		// --operators
