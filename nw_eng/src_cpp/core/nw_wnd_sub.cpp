...
namespace NW
{
	wnd_sub::wnd_sub(wnd_core& core, cinfo& information) :
		a_wnd(information),
		m_core(&core)
	{
		// register a window class to create a window; ModuleHandle is the current application;
		m_class.lpfnWndProc = wnd_sub::event_proc_init;
	...
	// --==</core_methods>==--
	// --==<impl_methods>==--
	LRESULT inline __stdcall wnd_sub::event_proc_init(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
		if (msg != WM_NCCREATE) { return ::DefWindowProc(hwnd, msg, wparam, lparam); }

		CREATESTRUCT* crtst = reinterpret_cast<CREATESTRUCT*>(lparam);
		wnd_sub* nw_window = reinterpret_cast<wnd_sub*>(crtst->lpCreateParams);

		::SetWindowLongPtr(hwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(nw_window));
		::SetWindowLongPtr(hwnd, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(wnd_sub::event_proc_static));

		return wnd_sub::event_proc_static(hwnd, msg, wparam, lparam);
	}
	LRESULT inline __stdcall wnd_sub::event_proc_static(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
		return reinterpret_cast<wnd_sub*>(::GetWindowLongPtr(hwnd, GWLP_USERDATA))->event_proc(hwnd, msg, wparam, lparam);
	}
	inline LRESULT __stdcall wnd_sub::event_proc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
		switch (msg) {
		// events_of_window
		{
		case WM_SIZE: {
