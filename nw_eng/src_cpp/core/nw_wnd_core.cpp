#include <nw_pch.hpp>
#include "nw_wnd_core.h"
#if (defined NW_WAPI)
#include <core/nw_engine.h>
#include "nw_wnd_sub.h"
#if (NW_WAPI & NW_WAPI_WIN)
#include <shellapi.h>
namespace NW
{
	wnd_core::wnd_core(cinfo& information) :
		a_wnd(information),
		m_gfx(graphics()), m_kbd(keyboard()), m_mouse(mouse())
	{
		...
		m_class.lpfnWndProc = wnd_core::event_proc_init;
	...
	void wnd_core::set_cursor_enabled(bit enable_cursor) {
		m_mouse.set_cursor_enabled(enable_cursor);
		gui_set_mouse_enabled(enable_cursor);
	}
	// --==<core_methods>==--
	void wnd_core::update()
	{
		m_gfx->update();
		m_mouse.update();
		m_kbd.update();
	...
	// --==</core_methods>==--
	// --==<impl_methods>==--
	LRESULT inline __stdcall wnd_core::event_proc_init(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
		...
		::SetWindowLongPtr(hwnd, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(wnd_core::event_proc_static));

		return wnd_core::event_proc_static(hwnd, msg, wparam, lparam);
	}
	LRESULT inline __stdcall wnd_core::event_proc_static(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
		if (gui_wapi_event_proc(hwnd, msg, wparam, lparam) == TRUE) { return 0l; }
		return reinterpret_cast<wnd_core*>(::GetWindowLongPtr(hwnd, GWLP_USERDATA))->event_proc(hwnd, msg, wparam, lparam);
	}
	inline LRESULT __stdcall wnd_core::event_proc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
		switch (msg) {
