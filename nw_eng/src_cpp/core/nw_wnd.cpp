...
namespace NW
{
	window_info::window_info(cstr window_title, v1ui width, v1ui height) :
		title(window_title),
...
namespace NW
{
	...
	// --setters
	void a_wnd::set_title(cstr window_title) {
		m_info.title = window_title;
		::SetWindowTextA(m_handle, window_title);
	}
	void a_wnd::set_opacity(v1f opacity) {
	...
	// --==</core_methods>==--
	// --==<impl_methods>==--
	LRESULT __stdcall a_wnd::event_proc_init(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
		if (msg != WM_NCCREATE) { return ::DefWindowProc(hwnd, msg, wparam, lparam); }

		CREATESTRUCT* crtst = reinterpret_cast<CREATESTRUCT*>(lparam);
		a_wnd* nw_window = reinterpret_cast<a_wnd*>(crtst->lpCreateParams);

		::SetWindowLongPtr(hwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(nw_window));
		::SetWindowLongPtr(hwnd, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(a_wnd::event_proc_static));

		return a_wnd::event_proc_static(hwnd, msg, wparam, lparam);
	}
	LRESULT __stdcall a_wnd::event_proc_static(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
		return reinterpret_cast<a_wnd*>(::GetWindowLongPtr(hwnd, GWLP_USERDATA))->event_proc(hwnd, msg, wparam, lparam);
	}
	LRESULT inline __stdcall a_wnd::event_proc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
		switch (msg) {
