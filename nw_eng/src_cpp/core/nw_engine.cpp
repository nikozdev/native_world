...
namespace NW
{
	...
	bool core_engine::init()
	{
		...
		m_wnd.make_ref<wnd_core>(window_info{ &m_name[0], 1200, 800 });
	...
	dstr core_engine::dialog_save(cstr filter) { return data_sys::get().save_dlg(filter, m_wnd->get_handle()); }
	dstr core_engine::dialog_load(cstr filter) { return data_sys::get().load_dlg(filter, m_wnd->get_handle()); }
	// --==</core_methods>==--
}