...
const char lua_ident[] = "$LuaVersion: " NW_LUA_COPYRIGHT " $" "$LuaAuthors: " NW_LUA_AUTHORS " $";
...
#define ispseudo(i)		((i) <= NW_LUA_REGISTRYINDEX)
// test for upvalue
#define isupvalue(i)		((i) < NW_LUA_REGISTRYINDEX)
...
static TValue* index2value(lua_State* L, int idx) {
    ...
    else if (idx == NW_LUA_REGISTRYINDEX)
        return &G(L)->l_registry;
    else {  /* upvalues */
        idx = NW_LUA_REGISTRYINDEX - idx;
...
NW_LUA_API int lua_checkstack(lua_State* L, int n) {
    ...
    if (L->stack_last - L->top > n) { res = 1; }  // yes; check is OK;
        ...
        if (inuse > NW_LUAI_MAXSTACK - n) res = 0;     // can grow without overflow? - no;
...
NW_LUA_API void lua_settop(lua_State* L, int idx) {
    ...
    if (diff < 0 && hastocloseCfunc(ci->nresults))
        luaF_close(L, L->top + diff, NW_LUA_OK);
...
NW_LUA_API void lua_copy(lua_State* L, int fromidx, int toidx) {
    ...
    // NW_LUAREGISTRYINDEX does not need gc barrier (collector revisits it before finishing collection);
    lua_unlock(L);
}
...
NW_LUA_API int lua_type(lua_State* L, int idx) {
    ...
    return (isvalid(L, o) ? ttype(o) : NW_LUA_TNONE);
}

NW_LUA_API const char* lua_typename(lua_State* L, int t) {
    UNUSED(L);
    api_check(L, NW_LUA_TNONE <= t && t < NW_LUA_NUMTYPES, "invalid type");
    return ttypename(t);
}
...
NW_LUA_API int lua_rawequal(lua_State* L, int index1, int index2) {
...
NW_LUA_API void lua_arith(lua_State* L, int op) {
    lua_lock(L);
    if (op != NW_LUA_OPUNM && op != NW_LUA_OPBNOT)
...
NW_LUA_API int lua_compare(lua_State* L, int index1, int index2, int op) {
    const TValue* o1;
    const TValue* o2;
    int i = 0;
    ...
    if (isvalid(L, o1) && isvalid(L, o2)) {
        switch (op) {
        case NW_LUA_OPEQ: i = luaV_equalobj(L, o1, o2); break;
        case NW_LUA_OPLT: i = luaV_lessthan(L, o1, o2); break;
        case NW_LUA_OPLE: i = luaV_lessequal(L, o1, o2); break;
...
NW_LUA_API lua_Number lua_tonumberx(lua_State* L, int idx, int* pisnum) {
    lua_Number n = 0;
    const TValue* o = index2value(L, idx);
...
NW_LUA_API lua_Integer lua_tointegerx(lua_State* L, int idx, int* pisnum) {
    lua_Integer res = 0;
    const TValue* o = index2value(L, idx);
...
NW_LUA_API lua_Unsigned lua_rawlen(lua_State* L, int idx) {
    const TValue* o = index2value(L, idx);
    switch (ttypetag(o)) {
    case NW_LUA_VSHRSTR: return tsvalue(o)->shrlen;
    case NW_LUA_VLNGSTR: return tsvalue(o)->u.lnglen;
    case NW_LUA_VUSERDATA: return uvalue(o)->len;
    case NW_LUA_VTABLE: return luaH_getn(hvalue(o));
    default: return 0;
    }
}
...
static void* touserdata(const TValue* o) {
    switch (ttype(o)) {
    case NW_LUA_TUSERDATA: return getudatamem(uvalue(o));
    case NW_LUA_TLIGHTUSERDATA: return pvalue(o);
    default: return NULL;
    }
}
...
NW_LUA_API const void* lua_topointer(lua_State* L, int idx) {
    const TValue* o = index2value(L, idx);
    switch (ttypetag(o)) {
    case NW_LUA_VLCF: return cast_voidp(cast_sizet(fvalue(o)));
    case NW_LUA_VUSERDATA: case NW_LUA_VLIGHTUSERDATA:
        return touserdata(o);
    default: {
        if (iscollectable(o))
            return gcvalue(o);
...
NW_LUA_API int lua_getglobal (lua_State *L, const char *name) {
  ...
  return auxgetstr(L, luaH_getint(reg, NW_LUA_RIDX_GLOBALS), name);
}
...
NW_LUA_API void lua_createtable (lua_State *L, int narray, int nrec) {
  Table *t;
  lua_lock(L);
  t = luaH_new(L);
...
NW_LUA_API void lua_setglobal (lua_State *L, const char *name) {
  ...
  reg = hvalue(&G(L)->l_registry);
  auxsetstr(L, luaH_getint(reg, NW_LUA_RIDX_GLOBALS), name);
}
...
#define checkresults(L,na,nr) \
     api_check(L, (nr) == NW_LUA_MULTRET || (L->ci->top - L->top >= (nr) - (na)), \
	"results from function overflow current stack size")


NW_LUA_API void lua_callk (lua_State *L, int nargs, int nresults,
  ...
  api_check(L, L->status == NW_LUA_OK, "cannot do calls on non-normal thread");
...
NW_LUA_API int lua_pcallk (lua_State *L, int nargs, int nresults, int errfunc,
                        lua_KContext ctx, lua_KFunction k) {
  struct CallS c;
  int status;
  ptrdiff_t func;
  ...
  if (k == NULL || !yieldable(L)) {  /* no continuation or no yieldable? */
    ...
    status = NW_LUA_OK;  /* if it is here, there were no errors */
  }
  adjustresults(L, nresults);
  lua_unlock(L);
  return status;
...
NW_LUA_API int lua_load (lua_State *L, lua_Reader reader, void *data,
  ...
  if (status == NW_LUA_OK) {  /* no errors? */
    LClosure *f = clLvalue(s2v(L->top - 1));  /* get newly created function */
    if (f->nupvalues >= 1) {  /* does it have an upvalue? */
      /* get global table from registry */
      Table *reg = hvalue(&G(L)->l_registry);
      const TValue *gt = luaH_getint(reg, NW_LUA_RIDX_GLOBALS);
      /* set global table as 1st upvalue of 'f' (may be NW_LUA_ENV) */
...
NW_LUA_API int lua_gc (lua_State *L, int what, ...) {
  ...
  switch (what) {
    ...
    case NW_LUA_GCRESTART: {
      luaE_setdebt(g, 0);
      g->gcrunning = 1;
      break;
    }
    case NW_LUA_GCCOLLECT: {
      luaC_fullgc(L, 0);
      break;
    }
    case NW_LUA_GCCOUNT: {
    ...
    case NW_LUA_GCSETPAUSE: {
      int data = va_arg(argp, int);
      res = getgcparam(g->gcpause);
      setgcparam(g->gcpause, data);
      break;
    }
    case NW_LUA_GCSETSTEPMUL: {
      int data = va_arg(argp, int);
      res = getgcparam(g->gcstepmul);
      setgcparam(g->gcstepmul, data);
      break;
    }
    case NW_LUA_GCISRUNNING: {
      res = g->gcrunning;
      break;
    }
    case NW_LUA_GCGEN: {
      ...
      res = isdecGCmodegen(g) ? NW_LUA_GCGEN : NW_LUA_GCINC;
      if (minormul != 0)
...
NW_LUA_API void *lua_newuserdatauv (lua_State *L, size_t size, int nuvalue) {
  Udata *u;
  lua_lock(L);
