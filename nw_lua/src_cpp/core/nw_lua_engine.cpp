...
namespace NW_LUA
{
	...
	// --==<core_methods>==--
	//nw_lua_table_info Luacmp_sys::lrtcmp_sys = lua_table_info("cmp_sys");
	//nw_lua_table_info Luaent_sys::lrtent_sys = lua_table_info("ent_sys");
	lua_state* lua_engine::new_lua_state()
	{
		lua_state* lstate = lua_newstate(lua_engine::lua_alloc , &m_MemAllocator);
		//nw_lua_state* pLState = luaL_newstate();
		//nw_lua_setallocf(pLState, lua_engine::LuaAllocate, &m_MemAllocator);
	...
	bool lua_engine::load_script(const dstr& script)
	{
		...
		v1s resultult = 0;
		//nw_lua_load(m_lstate, lua_engine::lua_read, &lstate_reader, "lua_chunk", "r");
		
		return resultult == NW_LUA_OK;
	}
	...
	bool lua_engine::run_script_str(cstr script)
	{
		...
		m_info.add_type(lua_type(m_lstate, -1));
		if (lua_pcallk(m_lstate, 0, 0, 0, 0, 0) != NW_LUA_OK) {
	...
	{
		bool result = true;
		if (luaL_dofile(m_lstate, &file_path[0]) != NW_LUA_OK) {
			write_err("could not run the script");
			result = false;
		}
		else { result = true; }
	...
	bool lua_engine::call_lua_func_glb(cstr name)
	{
		...
		if (result) { result = (lua_pcallk(m_lstate, 0, 0, 0, 0, 0) == NW_LUA_OK); lua_clear(1); }
	...
	bool lua_engine::call_lua_func_loc(cstr name)
	{
		...
		if (bResult) { bResult = (lua_pcallk(m_lstate, 0, 0, 0, 0, 0) == NW_LUA_OK); lua_clear(1); }
		else { write_err("function \"{str}\" was not found;", name); }
		return bResult;
	}
	bool lua_engine::call_lua_func(cstr strLoc, cstr name)
	{
		...
		if (bResult) { bResult = (lua_pcallk(m_lstate, 0, 0, 0, 0, 0) == NW_LUA_OK); lua_clear(); }
		else { write_err("function \"{str}\" was not found;", name); }
		return bResult;
	}
	...
	inline bool lua_engine::lua_push(lua_table_info& r_reg_tab)
	{
		...
		if (r_reg_tab.meta_table != nullptr) {
			...
			lua_setfield(m_lstate, NW_LUA_REGISTRYINDEX, r_reg_tab.meta_table->name);  // registry.name = metatable
...
//	bool bHasTab = lua_type(m_lstate, -1) == NW_LUA_TTABLE;
//	lua_clear(lvl_num);
//	return bHasTab;
//}
...
//	ui16 lvl_num = lua_move(name);
//	bool bHasFunc = lua_type(m_lstate, -1) == NW_LUA_TFUNCTION;
//	lua_clear(lvl_num);
//	return bHasFunc;
//}
...
//		if (lua_type(m_lstate, -1) == NW_LUA_TTABLE) {		// The over table is on the stack
