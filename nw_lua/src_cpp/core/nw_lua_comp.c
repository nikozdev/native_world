...
static const Proto* combine(lua_State* L, int n)
{
 ...
 else
 {
  Proto* f;
  int i=n;
  if (lua_load(L,reader,&i,"=(" PROGNAME ")",NULL)!=NW_LUA_OK) fatal(lua_tostring(L,-1));
  f=toproto(L,-1);
  for (i=0; i<n; i++)
  {
...
static int pmain(lua_State* L)
{
 ...
 for (i=0; i<argc; i++)
 {
  ...
  if (luaL_loadfile(L,filename)!=NW_LUA_OK) fatal(lua_tostring(L,-1));
 }
 f=combine(L,argc);
...
int main(int argc, char* argv[])
{
 ...
 if (lua_pcall(L,2,0,0)!=NW_LUA_OK) fatal(lua_tostring(L,-1));
 lua_close(L);
 return EXIT_SUCCESS;
}
...
static void PrintConstant(const Proto* f, int i)
{
 ...
 switch (ttypetag(o))
 {
  ...
  case NW_LUA_VNUMFLT:
	{
	char buff[100];
	sprintf(buff,NW_LUA_NUMBER_FMT,fltvalue(o));
	printf("%s",buff);
	if (buff[strspn(buff,"-0123456789")]=='\0') printf(".0");
	break;
	}
  case NW_LUA_VNUMINT:
	printf(NW_LUA_INTEGER_FMT,ivalue(o));
	break;
  case NW_LUA_VSHRSTR:
  case NW_LUA_VLNGSTR:
