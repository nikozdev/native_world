...
static int findfield (lua_State *L, int objidx, int level) {
  ...
  while (lua_next(L, -2)) {  /* for each pair in table */
    if (lua_type(L, -2) == NWLUA_TSTRING) {  /* ignore non-string keys */
...
static int pushglobalfuncname (lua_State *L, lua_Debug *ar) {
  ...
  if (findfield(L, top + 1, 2)) {
    ...
    if (strncmp(name, NWLUA_GNAME ".", 3) == 0) {  /* name start with '_G.'? */
...
int luaL_typeerror (lua_State *L, int arg, const char *tname) {
  ...
  if (luaL_getmetafield(L, arg, "__name") == NWLUA_TSTRING)
    typearg = lua_tostring(L, -1);  /* use the given type name */
  else if (lua_type(L, arg) == NWLUA_TLIGHTUSERDATA)
...
*/
NWLUA_API void luaL_where (lua_State *L, int level) {
  lua_Debug ar;
...
NWLUA_API int luaL_newmetatable (lua_State *L, const char *tname) {
  if (luaL_getmetatable(L, tname) != NWLUA_TNIL)  /* name already in use? */
  ...
  lua_pushvalue(L, -1);
  lua_setfield(L, NWLUA_REGISTRYINDEX, tname);  /* registry.name = metatable */
  return 1;
}
...
** but without 'msg'.)
*/
NWLUA_API void luaL_checkstack (lua_State *L, int space, const char *msg) {
...
NWLUA_API void luaL_checkany (lua_State *L, int arg) {
  if (lua_type(L, arg) == NWLUA_TNONE)
    luaL_argerror(L, arg, "value expected");
}


NWLUA_API const char *luaL_checklstring (lua_State *L, int arg, size_t *len) {
  const char *s = lua_tolstring(L, arg, len);
  if (!s) tag_error(L, arg, NWLUA_TSTRING);
  return s;
}
...
NWLUA_API int luaL_ref (lua_State *L, int t) {
  ...
  if (lua_isnil(L, -1)) {
    ...
    return NWLUA_REFNIL;  /* 'nil' has a unique fixed reference */
  }
  t = lua_absindex(L, t);
...
NWLUA_API int luaL_loadfilex (lua_State *L, const char *filename,
  ...
  if (c == NWLUA_SIGNATURE[0] && filename) {  /* binary file? */
...
NWLUA_API int luaL_loadbufferx (lua_State *L, const char *buff, size_t size,
...
NWLUA_API int luaL_getmetafield (lua_State *L, int obj, const char *event) {
  if (!lua_getmetatable(L, obj))  /* no metatable? */
    ...
    tt = lua_rawget(L, -2);
    if (tt == NWLUA_TNIL)  /* is metafield nil? */
...
NWLUA_API int luaL_callmeta (lua_State *L, int obj, const char *event) {
  obj = lua_absindex(L, obj);
  if (luaL_getmetafield(L, obj, event) == NWLUA_TNIL)  /* no metafield? */
    return 0;
  lua_pushvalue(L, obj);
  lua_call(L, 1, 1);
  return 1;
    ...
    switch (lua_type(L, idx)) {
      case NWLUA_TNUMBER: {
        if (lua_isinteger(L, idx))
          lua_pushfstring(L, "%I", (NWLUA_UINT)lua_tointeger(L, idx));
        else
          lua_pushfstring(L, "%f", (NWLUAI_UACNUMBER)lua_tonumber(L, idx));
        break;
      }
      case NWLUA_TSTRING:
        lua_pushvalue(L, idx);
      ...
      default: {
        ...
        const char *kind = (tt == NWLUA_TSTRING) ? lua_tostring(L, -1) :
                                                 luaL_typename(L, idx);
...
NWLUA_API int luaL_getsubtable (lua_State *L, int idx, const char *fname) {
  if (lua_getfield(L, idx, fname) == NWLUA_TTABLE)
...
NWLUA_API void luaL_checkversion_ (lua_State *L, lua_Number ver, size_t sz) {
  ...
  if (sz != NWLUAL_NUMSIZES)  /* check numeric types */
    luaL_error(L, "core and library have incompatible numeric types");
  else if (v != ver)
    luaL_error(L, "version mismatch: app. needs %f, Lua core provides %f",
                  (NWLUAI_UACNUMBER)ver, (NWLUAI_UACNUMBER)v);
}
