...
#define noLuaClosure(f)		((f) == NULL || (f)->c.tt == NWLUA_VCCL)
...
static const char *gxf (const Proto *p, int pc, Instruction i, int isup) {
  ...
  return (name && strcmp(name, NWLUA_ENV) == 0) ? "global" : "field";
}
...
int luaG_traceexec (lua_State *L, const Instruction *pc) {
  ...
  if (!(mask & (NWLUA_MASKLINE | NWLUA_MASKCOUNT))) {  /* no hooks? */
    ci->u.l.trap = 0;  /* don't need to stop again */
    return 0;  /* turn off 'trap' */
  }
  pc++;  /* reference is always next instruction */
  ci->u.l.savedpc = pc;  /* save 'pc' */
  counthook = (--L->hookcount == 0 && (mask & NWLUA_MASKCOUNT));
  if (counthook)
    resethookcount(L);  /* reset count */
  else if (!(mask & NWLUA_MASKLINE))
  ...
  if (counthook)
    luaD_hook(L, NWLUA_HOOKCOUNT, -1, 0, 0);  /* call count hook */
  if (mask & NWLUA_MASKLINE) {
    int npci = pcRel(pc, p);
    if (npci == 0 ||  /* call linehook when enter a new function, */
        pc <= invpcRel(oldpc, p) ||  /* when jump back (loop), or when */
        changedline(p, oldpc, npci)) {  /* enter new line */
      int newline = luaG_getfuncline(p, npci);
      luaD_hook(L, NWLUA_HOOKLINE, newline, 0, 0);  /* call line hook */
    }
    L->oldpc = npci;  /* 'pc' of last call to line hook */
  }
  if (L->status == NWLUA_YIELD) {  /* did hook yield? */
    if (counthook)
