...
const char lua_ident[] = "$LuaVersion: " LUA_COPYRIGHT " $" "$LuaAuthors: " LUA_AUTHORS " $";
...
LUA_API int lua_checkstack(lua_State* L, int n) {
    ...
    if (L->stack_last - L->top > n) { res = 1; }  // yes; check is OK;
        ...
        if (inuse > LUAI_MAXSTACK - n) res = 0;     // can grow without overflow? - no;
...
LUA_API size_t lstringtonumber(lua_State* L, const char* s) {
