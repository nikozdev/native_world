...
static GCObject **getgclist (GCObject *o) {
  switch (o->tt) {
    case NW_LUA_VTABLE: return &gco2t(o)->gclist;
    case NW_LUA_VLCL: return &gco2lcl(o)->gclist;
    case NW_LUA_VCCL: return &gco2ccl(o)->gclist;
    case NW_LUA_VTHREAD: return &gco2th(o)->gclist;
    case NW_LUA_VPROTO: return &gco2p(o)->gclist;
...
static int iscleared (global_State *g, const GCObject *o) {
  ...
  else if (novariant(o->tt) == NW_LUA_TSTRING) {
...
static void reallymarkobject (global_State *g, GCObject *o) {
  switch (o->tt) {
    ...
    case NW_LUA_VLCL: case NW_LUA_VCCL: case NW_LUA_VTABLE:
    case NW_LUA_VTHREAD: case NW_LUA_VPROTO: {
...
static lu_mem propagatemark (global_State *g) {
  ...
  switch (o->tt) {
    case NW_LUA_VTABLE: return traversetable(g, gco2t(o));
    case NW_LUA_VUSERDATA: return traverseudata(g, gco2u(o));
    case NW_LUA_VLCL: return traverseLclosure(g, gco2lcl(o));
    case NW_LUA_VCCL: return traverseCclosure(g, gco2ccl(o));
    case NW_LUA_VPROTO: return traverseproto(g, gco2p(o));
    case NW_LUA_VTHREAD: return traversethread(g, gco2th(o));
    default: lua_assert(0); return 0;
  }
}
...
static void GCTM (lua_State *L) {
  ...
  if (!notm(tm)) {  /* is there a finalizer? */
    ...
    if (unlikely(status != NW_LUA_OK)) {  /* error while running __gc? */
  ...
  while ((curr = *p) != NULL) {
    if (iswhite(curr)) {  /* is 'curr' dead? */
      ...
      *p = curr->next;  /* remove 'curr' from list */
      ...
      setage(curr, G_OLD);
      if (curr->tt == NW_LUA_VTHREAD) {  /* threads must be watched */
        lua_State *th = gco2th(curr);
        linkgclist(th, g->grayagain);  /* insert into 'grayagain' list */
      }
      else if (curr->tt == NW_LUA_VUPVAL && upisopen(gco2upv(curr)))
...
static GCObject **correctgraylist (GCObject **p) {
  ...
  while ((curr = *p) != NULL) {
    ...
    else if (curr->tt == NW_LUA_VTHREAD) {
      lua_assert(isgray(curr));
