...
const char lua_ident[] = "$LuaVersion: " NWLUA_COPYRIGHT " $" "$LuaAuthors: " NWLUA_AUTHORS " $";
...
#define ispseudo(i)		((i) <= NWLUA_REGISTRYINDEX)
// test for upvalue
#define isupvalue(i)		((i) < NWLUA_REGISTRYINDEX)
...
static TValue* index2value(lua_State* L, int idx) {
    ...
    else if (idx == NWLUA_REGISTRYINDEX)
        return &G(L)->l_registry;
    else {  /* upvalues */
        idx = NWLUA_REGISTRYINDEX - idx;
...
NWLUA_API int lua_checkstack(lua_State* L, int n) {
    ...
    if (L->stack_last - L->top > n) { res = 1; }  // yes; check is OK;
        ...
        if (inuse > NWLUAI_MAXSTACK - n) res = 0;     // can grow without overflow? - no;
...
NWLUA_API void lua_settop(lua_State* L, int idx) {
    ...
    if (diff < 0 && hastocloseCfunc(ci->nresults))
        luaF_close(L, L->top + diff, NWLUA_OK);
...
NWLUA_API void lua_copy(lua_State* L, int fromidx, int toidx) {
    ...
    // NWLUAREGISTRYINDEX does not need gc barrier (collector revisits it before finishing collection);
    lua_unlock(L);
}
...
NWLUA_API int lua_type(lua_State* L, int idx) {
    ...
    return (isvalid(L, o) ? ttype(o) : NWLUA_TNONE);
}

NWLUA_API const char* lua_typename(lua_State* L, int t) {
    UNUSED(L);
    api_check(L, NWLUA_TNONE <= t && t < NWLUA_NUMTYPES, "invalid type");
    return ttypename(t);
}
...
NWLUA_API int lua_rawequal(lua_State* L, int index1, int index2) {
...
NWLUA_API void lua_arith(lua_State* L, int op) {
    lua_lock(L);
    if (op != NWLUA_OPUNM && op != NWLUA_OPBNOT)
...
NWLUA_API int lua_compare(lua_State* L, int index1, int index2, int op) {
    const TValue* o1;
    const TValue* o2;
    int i = 0;
    ...
    if (isvalid(L, o1) && isvalid(L, o2)) {
        switch (op) {
        case NWLUA_OPEQ: i = luaV_equalobj(L, o1, o2); break;
        case NWLUA_OPLT: i = luaV_lessthan(L, o1, o2); break;
        case NWLUA_OPLE: i = luaV_lessequal(L, o1, o2); break;
...
NWLUA_API lua_Number lua_tonumberx(lua_State* L, int idx, int* pisnum) {
    lua_Number n = 0;
    const TValue* o = index2value(L, idx);
...
NWLUA_API lua_Integer lua_tointegerx(lua_State* L, int idx, int* pisnum) {
    lua_Integer res = 0;
    const TValue* o = index2value(L, idx);
...
NWLUA_API lua_Unsigned lua_rawlen(lua_State* L, int idx) {
    const TValue* o = index2value(L, idx);
    switch (ttypetag(o)) {
    case NWLUA_VSHRSTR: return tsvalue(o)->shrlen;
    case NWLUA_VLNGSTR: return tsvalue(o)->u.lnglen;
    case NWLUA_VUSERDATA: return uvalue(o)->len;
    case NWLUA_VTABLE: return luaH_getn(hvalue(o));
    default: return 0;
    }
}
...
static void* touserdata(const TValue* o) {
    switch (ttype(o)) {
    case NWLUA_TUSERDATA: return getudatamem(uvalue(o));
    case NWLUA_TLIGHTUSERDATA: return pvalue(o);
    default: return NULL;
    }
}
...
NWLUA_API const void* lua_topointer(lua_State* L, int idx) {
    const TValue* o = index2value(L, idx);
    switch (ttypetag(o)) {
    case NWLUA_VLCF: return cast_voidp(cast_sizet(fvalue(o)));
    case NWLUA_VUSERDATA: case NWLUA_VLIGHTUSERDATA:
        return touserdata(o);
    default: {
        if (iscollectable(o))
            return gcvalue(o);
...
NWLUA_API int lua_getglobal (lua_State *L, const char *name) {
  ...
  return auxgetstr(L, luaH_getint(reg, NWLUA_RIDX_GLOBALS), name);
}
...
NWLUA_API void lua_createtable (lua_State *L, int narray, int nrec) {
  Table *t;
  lua_lock(L);
  t = luaH_new(L);
...
NWLUA_API void lua_setglobal (lua_State *L, const char *name) {
  ...
  reg = hvalue(&G(L)->l_registry);
  auxsetstr(L, luaH_getint(reg, NWLUA_RIDX_GLOBALS), name);
}
...
#define checkresults(L,na,nr) \
     api_check(L, (nr) == NWLUA_MULTRET || (L->ci->top - L->top >= (nr) - (na)), \
	"results from function overflow current stack size")


NWLUA_API void lua_callk (lua_State *L, int nargs, int nresults,
  ...
  api_check(L, L->status == NWLUA_OK, "cannot do calls on non-normal thread");
...
NWLUA_API int lua_pcallk (lua_State *L, int nargs, int nresults, int errfunc,
                        lua_KContext ctx, lua_KFunction k) {
  struct CallS c;
  int status;
  ptrdiff_t func;
  ...
  if (k == NULL || !yieldable(L)) {  /* no continuation or no yieldable? */
    ...
    status = NWLUA_OK;  /* if it is here, there were no errors */
  }
  adjustresults(L, nresults);
  lua_unlock(L);
  return status;
...
NWLUA_API int lua_load (lua_State *L, lua_Reader reader, void *data,
  ...
  if (status == NWLUA_OK) {  /* no errors? */
    LClosure *f = clLvalue(s2v(L->top - 1));  /* get newly created function */
    if (f->nupvalues >= 1) {  /* does it have an upvalue? */
      /* get global table from registry */
      Table *reg = hvalue(&G(L)->l_registry);
      const TValue *gt = luaH_getint(reg, NWLUA_RIDX_GLOBALS);
      /* set global table as 1st upvalue of 'f' (may be NWLUA_ENV) */
...
NWLUA_API int lua_gc (lua_State *L, int what, ...) {
  ...
  switch (what) {
    ...
    case NWLUA_GCRESTART: {
      luaE_setdebt(g, 0);
      g->gcrunning = 1;
      break;
    }
    case NWLUA_GCCOLLECT: {
      luaC_fullgc(L, 0);
      break;
    }
    case NWLUA_GCCOUNT: {
    ...
    case NWLUA_GCSETPAUSE: {
      int data = va_arg(argp, int);
      res = getgcparam(g->gcpause);
      setgcparam(g->gcpause, data);
      break;
    }
    case NWLUA_GCSETSTEPMUL: {
      int data = va_arg(argp, int);
      res = getgcparam(g->gcstepmul);
      setgcparam(g->gcstepmul, data);
      break;
    }
    case NWLUA_GCISRUNNING: {
      res = g->gcrunning;
      break;
    }
    case NWLUA_GCGEN: {
      ...
      res = isdecGCmodegen(g) ? NWLUA_GCGEN : NWLUA_GCINC;
      if (minormul != 0)
...
NWLUA_API void *lua_newuserdatauv (lua_State *L, size_t size, int nuvalue) {
  Udata *u;
  lua_lock(L);
