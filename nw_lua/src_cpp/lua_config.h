...
#	define LUA_DL_DLL	// enable support for dynamic library linking
...
#endif
// @@ LUAI_IS32INT is true if 'int' has (at least) 32 bits.
...
// int and float
#if defined(LUA_32BITS) // 32-bit integers and 'float'
...
// (define LUA_BUILD_AS_DLL to get it);
#if defined(NW_BUILD_DLL)
#	if defined(LUA_CORE) || defined(LUA_LIB)	 { 
#		define LUA_API __declspec(dlua_lexport)
#	else						 }{ 
...
// @@ LUAI_FUNC is a mark for all extern functions that are not to be exported to outside modules;
...
// use LUAI_UACINT here to avoid problems with promotions
...
#elif LUA_INT_TYPE == LUA_INT_LONGLONG // use presence of macro LLONG_MAX as proxy for C99 compliance;
...
#		error "Compiler does not support 'long long'. Use option '-DLUA_32BITS' \
	or '-DLUA_C89_NUMBERS' (see file 'lua_config.h' for details)"
#	endif
#else
...
// --==<language_variations>==--
// @@ LUA_NOCVTN2S/LUA_NOCVTS2N control how lua performs some convertions;
// define LUA_NOCVTN2S to turn off automatic convertion from numbers to strings;
// #define LUA_NOCVTN2S
// #define LUA_NOCVTS2N
...
#endif
// @@ LUA_EXTRASPACE defines the size of a raw memory area
